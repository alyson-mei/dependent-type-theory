...
\begin{example}[Addition for $\nat$]
    We define $\text{plus}: \nat \to \nat \to \nat$ as:
    $$\lambda x \lambda y. \iter(x, x'.\scc(x'))(y)$$
\end{example}
...
\begin{itemize}
    \item  Proposition $\sim$ Type,
    \item Predicate ("propositional function") $\sim$ Family of types ("typical function")
\end{itemize}
